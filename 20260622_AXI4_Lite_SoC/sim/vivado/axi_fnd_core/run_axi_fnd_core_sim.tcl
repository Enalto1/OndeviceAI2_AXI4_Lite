# Vivado 2020.2 batch simulation script for axi_fnd_core.
...
set sim_dir [file join $project_root sim vivado axi_fnd_core]
set result_file [file join $sim_dir axi_fnd_core_sim_result.txt]
set log_dir [file join $sim_dir logs]
file mkdir $log_dir
set stamp [clock format [clock seconds] -format "%Y%m%d_%H%M%S"]
set vivado_log_copy [file join $log_dir "axi_fnd_core_sim_${stamp}_vivado.log"]
set xsim_log_copy [file join $log_dir "axi_fnd_core_sim_${stamp}_xsim.log"]

set work_dir [file join $sim_dir vivado_work]
create_project axi_fnd_core_sim $work_dir -part xc7a35tcpg236-1 -force
set_property target_language Verilog [current_project]
set_property simulator_language Mixed [current_project]

add_files -norecurse [list \
    [file join $project_root rtl_work axi_peripherals axi_fnd_core hdl axi_fnd_core.v] \
    [file join $project_root axi_project_unique_sources sources fnd_controller.v] \
]

set tb_file [file join $project_root sim vivado axi_fnd_core tb_axi_fnd_core.sv]
add_files -fileset sim_1 -norecurse $tb_file
set_property file_type SystemVerilog [get_files $tb_file]
set_property top tb_axi_fnd_core [get_filesets sim_1]
...
set xsim_log_src [file join $work_dir axi_fnd_core_sim.sim sim_1 behav xsim simulate.log]
